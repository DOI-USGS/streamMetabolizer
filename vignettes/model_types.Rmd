...
```{r, eval=FALSE}
fit <- metab_mle(data=mydat, model_specs=specs_mle_obserr())
```

You can override the defaults by passing values to the `specs_` function; see the function's help file (e.g., `?specs_mle_obserr)` for the list of options and their definitions.
```{r, eval=FALSE}
?specs_mle_obserr
fit <- metab_mle(data=mydat, model_specs=specs_mle_obserr(GPP_init=7, K600_init=35))
```
...
fit <- metab_bayes(model_specs=specs_bayes_jags_nopool_procobserr())
```
