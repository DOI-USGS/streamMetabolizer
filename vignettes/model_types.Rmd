...
streamMetabolizer provides many options for customizing your metabolism model. At a very high level, these are grouped by their fitting method. You can choose from 'bayes', 'mle', or 'night'. All of these are fit with the same high-level function, `metab`.
```{r, eval=FALSE}
...
```{r, eval=FALSE}
fit <- metab_mle(data=mydat, specs=specs_mle_nopool_oi())
```
...
fit <- metab_mle(data=mydat, specs=specs('m_np_oi_pm_km.nlm', GPP_init=7, K600_init=35))
```
...
```{r, eval=2}
?mm_name
mm_name(type="bayes", engine="jags", pool_K600='none', err_obs_iid=TRUE)
```
