...
beaty_creek_tidy <- beaty_creek %>%
  rename(DO_mgL=ts_doobs, WaterTemp_C=ts_wtr, Discharge_cfs=ts_disch) %>%
  gather(variable, value, DO_mgL, WaterTemp_C, Discharge_cfs)  
...
beaty_creek_may15 <- beaty_creek %>% 
  rename(date.time=DateTime, DO.obs=ts_doobs) %>%
  ...
  mutate(DO.sat=calc_DO_at_sat(temp.water=ts_wtr, pressure.air=1000),
         ...
         light=calc_solar_insolation(jday=floor(doyhr), hour=24*(doyhr %% 1), latitude=site_location(beaty_creek_id)$latitude)) %>%
...
Here are the observed and modeled DO concentrations. Looks like maybe the timestamps are off on the observations?
```{r}
