...
This file demonstrates how to estimate stream metabolism using example data
supplied by Bob Hall for French Creek in Laramie, WY.
...
Load a moderately-sized example dataset from the package. Explicitly attached
units are optional but useful for confirming that the data are in the expected
format. We'll use the `streamMetabolizer` standard in defining our day to run
from 4 am (`day_start=4`) to 4 am (`day_end=28`).
```{r data}
...
Now inspect the dimensions, column names, and units of the data. Note that
...
Our data format matches that of the requirements from `metab_inputs`, so we're 
ready to go.
...
"b_np_oipi_pm_km.stan" and "m_np_oi_pm_km.nlm". You never need to remember these
concise names; instead, you can construct and interpret such names using the
`mm_name` and `mm_parse_name` functions, respectively.
...
and errors in attributes to the result of `predict_metab`.
```{r bayes_warning}
attr(preds, 'warnings')
attr(preds, 'errors')
```
