...
#'   accept the arguments \code{c(data, data_daily, ..., day_start, day_end, date)}
...
mm_model_by_ply <- function(data, data_daily, model_fun, day_start, day_end, ...) {
  ...
  out.all <- do.call(
    ...
    lapply(sort(unique.dates), function(dt) {
      ...
      out <- model_fun(
        data=data.plys[ply,!(names(data.plys) %in% c('odd.date.group','even.date.group'))], 
        data_daily=if(!missing(data_daily) && !is.null(data_daily)) data_daily[data_daily$local.date == dt,] else NULL,
        day_start=day_start, day_end=day_end, date=dt, ...)
      if(is.null(out) || nrow(out)==0) NULL else data.frame(date=dt, out)
    }))
