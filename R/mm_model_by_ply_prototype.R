...
#' mm_model_by_ply_prototype(extra_arg=1:7)
#' @export
mm_model_by_ply_prototype <- function(
  ...
  message("this function just returns the lengths of its inputs (for testing an mm_model_by_ply workflow)")
  all_args <- c(
    list(data_ply=data_ply, data_daily_ply=data_daily_ply, day_start=day_start, 
         day_end=day_end, ply_date=ply_date, ply_validity=ply_validity, timestep_days=timestep_days),
    list(...))
  as.data.frame(lapply(all_args, function(arg) length(arg)))
}