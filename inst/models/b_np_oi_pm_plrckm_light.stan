// b_np_oi_pm_km.stan

data {
  // Parameters of priors on metabolism
  real GPP_daily_mu;
  real GPP_daily_sigma;
  real ER_daily_mu;
  real ER_daily_sigma;
  real K600_daily_mu;
  real K600_daily_sigma;
  
  // Error distributions
  real err_obs_iid_sigma_shape;
  real err_obs_iid_sigma_rate;
  
  // Data dimensions
  int<lower=1> d; // number of dates
  int<lower=1> n; // number of observations per date
  
  // Daily data
  //vector[d] DO_obs_1;
  
  // Data
  vector[d] DO_obs[n];
  vector[d] DO_sat[n];
  vector[d] frac_GPP[n];
  vector[d] frac_ER[n];
  vector[d] frac_D[n];
  vector[d] depth[n];
  vector[d] KO2_conv[n];
}

transformed data {
  vector[d] coef_light[n-1]; // need light and depth separately for GPP with light process error
  vector[d] coef_depth[n-1];
  vector[d] coef_ER[n-1];
  vector[d] coef_K600_part[n-1];
  vector[d] DO_sat_pairmean[n-1];
  
  for(i in 1:(n-1)) {
    // Coefficients by pairmeans (e.g., mean(frac_GPP[i:(i+1)]) applies to the DO step from i to i+1)
    coef_light[i]  <- (frac_GPP[i] + frac_GPP[i+1])/2.0;
    coef_depth[i]  <- (depth[i] + depth[i+1])/2.0;
    coef_ER[i]   <- (frac_ER[i] + frac_ER[i+1])/2.0 ./ ((depth[i] + depth[i+1])/2.0);
    coef_K600_part[i] <- (KO2_conv[i] + KO2_conv[i+1])/2.0 .* (frac_D[i] + frac_D[i+1])/2.0;
    DO_sat_pairmean[i] <- (DO_sat[i] + DO_sat[i+1])/2.0;
  }
  
}

parameters {
  vector<lower=0, upper=50> [d] GPP_daily;
  vector<lower=-50,upper=0> [d] ER_daily;
  vector<lower=0, upper=200> [d] K600_daily;
  vector<lower=0>[d] mult_GPP[n-1]; // equal to bob's eta
  
  real<lower=0.01, upper=0.3> err_obs_iid_sigma;
  real<lower=0.1, upper=2> mult_GPP_sigma; // distribution parameter for mult_GPP
}

transformed parameters {
  vector[d] DO_mod[n];
  vector[d] coef_GPP[n-1];
  real mult_coef_GPP[n-1,d];
    
  for(j in 1:d) {
    for(i in 1:(n-1)) {
      mult_coef_GPP[i,j] <- mult_GPP[i,j] * coef_light[i,j];
    }
    for(i in 1:(n-1)) {
      coef_GPP[i,j] <- (mult_coef_GPP[i,j] / sum(mult_coef_GPP[1:(n-1),j])) / coef_depth[i,j];
    }
  }
  
  // Model DO time series
  // * pairmeans version
  // * observation error
  // * no process error
  // * reaeration depends on DO_mod
  
  // DO model
  DO_mod[1] <- DO_obs[1];
  for(i in 1:(n-1)) {
    DO_mod[i+1] <- (
      DO_mod[i] +
      GPP_daily .* coef_GPP[i] +
      ER_daily .* coef_ER[i] +
      K600_daily .* coef_K600_part[i] .* (DO_sat_pairmean[i] - DO_mod[i]/2.0)
    ) ./ (1.0 + K600_daily .* coef_K600_part[i] / 2.0);
  }
}

model {
  // Independent, identically distributed observation error
  for(i in 2:n) {
    DO_obs[i] ~ normal(DO_mod[i], err_obs_iid_sigma);
  }
  // SD (sigma) of the observation errors
  err_obs_iid_sigma ~ gamma(err_obs_iid_sigma_shape, err_obs_iid_sigma_rate);
  
  // Light multiplier to capture error in the GPP rate
  for(i in 1:(n-1)) {
    mult_GPP[i] ~ lognormal(0, 1); //mult_GPP_sigma
  }
  // SD (sigma) of the light multiplier
  //mult_GPP_sigma ~ gamma(1, 0.1);
  
  // Daily metabolism priors
  GPP_daily ~ normal(GPP_daily_mu, GPP_daily_sigma);
  ER_daily ~ normal(ER_daily_mu, ER_daily_sigma);
  K600_daily ~ normal(K600_daily_mu, K600_daily_sigma);
}
