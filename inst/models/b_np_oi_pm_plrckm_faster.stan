...
data {
  ...
  real err_obs_iid_sigma_rate; # using as a lognormal scaling factor now
...
parameters {
  vector[d] GPP_daily_scaled;
  vector[d] ER_daily_scaled;
  vector[d] K600_daily_scaled;
...
transformed parameters {
  ...
  // Scaled parameters
  GPP_daily <- GPP_daily_mu + (GPP_daily_sigma * GPP_daily_scaled);
  ER_daily <- ER_daily_mu + (ER_daily_sigma * ER_daily_scaled);
  K600_daily <- K600_daily_mu + (K600_daily_sigma * K600_daily_scaled);
...
model {
  ...
  GPP_daily_scaled ~ normal(0, 1);
  ER_daily_scaled ~ normal(0, 1);
  K600_daily_scaled ~ normal(0, 1);
}
