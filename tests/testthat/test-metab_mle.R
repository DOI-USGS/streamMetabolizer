...
test_that("metab_mle models can be created", {
  ...
  expect_warning(mm <- metab_mle(data=vfrench), 'temperature out of range')
...
test_that("metab_mle predictions (predict_metab, predict_DO) make sense", {
  ...
  mmE <- metab_mle(data=vfrenchshort, day_start=-1, day_end=23, specs=specs('m_np_oi_eu_km.nlm'))
  mmP <- metab_mle(data=vfrenchshort, day_start=-1, day_end=23, specs=specs('m_np_oi_pm_km.nlm'))
