...
test_that("metab_Kmodel predictions (predict_metab, predict_DO) make sense", {
  ...
  expect_warning(mm <- metab_Kmodel(data=NULL, data_daily=frk, model_specs=specs(mm_name("Kmodel", engine='mean'))), "no SE available")
  expect_equal(predict_metab(mm)$K600, frk$K600)
  # show that Kmodel(lm) and Kmodel(loess) do break, on model-specific errors
  expect_error(metab_Kmodel(data=fr, data_daily=frk, model_specs=specs(mm_name("Kmodel", engine='lm'))), "Error in lm.wfit", fixed=TRUE)
  expect_error(metab_Kmodel(data=fr, data_daily=frk, model_specs=specs(mm_name("Kmodel", engine='loess'))), "Error in simpleLoess", fixed=TRUE)
  
  # mean
  expect_warning(mm_mean <- metab_Kmodel(data_daily=frk2, model_specs=specs(mm_name('Kmodel', engine='mean'))), "no SE available")
  
  # lm
  ms <- specs(mm_name('Kmodel', engine='lm'), predictors=c())
  mm <- metab_Kmodel(data_daily=frk2, model_specs=ms)
  expect_equal(predict_metab(mm_mean)$K600, predict_metab(mm)$K600)
  mm <- metab_Kmodel(data_daily=frk2, model_specs=replace(ms, 'predictors', 'discharge.daily'))
  mm <- metab_Kmodel(data_daily=frk2, model_specs=replace(ms, 'predictors', 'date'))
  mm <- metab_Kmodel(data_daily=frk2, model_specs=specs(mm_name('Kmodel', engine='lm'), predictors=c('discharge.daily','date')))
  
  # loess
  mm <- metab_Kmodel(data_daily=frk2, model_specs=specs(mm_name('Kmodel', engine='loess'), predictors='date'))
  mm <- metab_Kmodel(data_daily=frk2, model_specs=specs(mm_name('Kmodel', engine='loess'), predictors='date', other_args=list(span=1.4)))
  mm <- metab_Kmodel(data_daily=frk2,model_specs=specs(mm_name('Kmodel', engine='loess'), predictors=c('discharge.daily','date')))
  mm <- metab_Kmodel(data_daily=frk2, model_specs=specs(mm_name('Kmodel', engine='loess'), predictors='discharge.daily', other_args=list(span=2)))
...
test_that("try a complete PRK-K-PR workflow", {
  ...
  # smooth the K600s
  expect_warning({mm2 <- metab_Kmodel(data_daily=K600_mm1, model_specs=specs(
