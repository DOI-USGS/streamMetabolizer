...
test_that("mm_model_by_ply", {
  ...
  dummy_middle_fun <- function(
    data_ply, data_daily_ply, day_start, day_end, local_date, # supplied by mm_model_by_ply; inheritParams mm_model_by_ply_prototype
    favorite # inheritParams dummy_inner_fun
  ) {
    ...
    if(nrow(data_ply) > 0 && nrow(data_daily_ply) > 0) {
      expect_true(data_daily_ply$local.date %in% unique(as.Date(data_ply$local.time)))
    }
    
    # fix up empty daily data because we can do that here (or in dummy_inner_fun)
    if(nrow(data_daily_ply) == 0) data_daily_ply <- data.frame(local.date=local_date, daily_row=NA)
  ...
  dummy_inner_fun <- function(inst_dat, daily_dat, favorite) {
    ...
    if(nrow(inst_dat) == 0) inst_dat <- data.frame(local.time=as.POSIXct(as.character(daily_dat$local.date), tz='UTC'), inst_row=NA)
  ...
  # define input data
  dat <- data.frame(local.time=(as.POSIXct(as.character(Sys.Date()), tz='UTC') + as.difftime(1:(24*7), units="hours")), inst_row=1:(24*7))
  datd <- data.frame(local.date=(Sys.Date() + as.difftime(c(-2,-1,1:3,5), units="days")), daily_row=1:6) #intentionally create date mismatch w/ dat
  ...
  expect_equal(as.Date(out$inst.local.time), out$daily.local.date) # daily & inst data should be matched up right by date
  expect_equal(out$local.date, out$daily.local.date) # mm_model_by_ply date & daily date should be equal
  expect_equal(missing_dailies, unique(out[is.na(out$daily.daily_row),'local.date'])) # inst obs w/o daily data should be kept
  expect_false(any(extra_dailies %in% unique(out$local.date))) # daily data w/o inst obs should be removed
  out_hours <- as.numeric(format(out$inst.local.time, "%H"))
  ...
  expect_true(all(as.Date(out$inst.local.time) >= out$local.date - as.difftime(1, units="days") & 
                    as.Date(out$inst.local.time) <= out$local.date + as.difftime(1, units="days"))) # were daily & inst data matched up right?
  out_hours <- as.numeric(format(out$inst.local.time, "%H"))
