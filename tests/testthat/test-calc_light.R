...
test_that("calc_solar_insolation has consistent output with that of calc_sun_rise_set and calc_is_daytime", {
  ...
  insdf <- data.frame(
    ...
    mutate(
      solar.date=as.Date(sprintf("2000-%d",jday), format="%Y-%j"),
      app.solar.time=as.POSIXct(strptime(sprintf("2000-%d 00",jday), format="%Y-%j %H"), tz="GMT"),
      lm_sunrise=calc_sun_rise_set(solar.date, lat)[,1],
      lm_sunset=calc_sun_rise_set(solar.date, lat)[,2]) %>%
    group_by(jday, lat) %>%
    do(with(., {
